...
public final class CodeTypeDemoCommand implements BotCommand {
  private static final Logger logger = LogManager.getLogger(CodeTypeDemoCommand.class);
  ...
  public Optional<Value> execute(GlobalSessionContext globalSessionContext,
    ...
    CodeTypeDemo command = new CodeTypeDemo();
    ...
    if(parameters.containsKey("firstName") && parameters.get("firstName") != null && parameters.get("firstName").get() != null) {
      ...
      if(convertedParameters.get("firstName") !=null && !(convertedParameters.get("firstName") instanceof String)) {
    ...
    try {
      command.runCode((String)convertedParameters.get("firstName"));Optional<Value> result = Optional.empty();
      return logger.traceExit(result);
    }
    catch (ClassCastException e) {
      throw new BotCommandException(MESSAGES_GENERIC.getString("generic.IllegalParameters","runCode"));
    }
