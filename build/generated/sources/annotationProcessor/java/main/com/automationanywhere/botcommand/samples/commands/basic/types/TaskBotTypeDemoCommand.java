...
public final class TaskBotTypeDemoCommand implements BotCommand {
  private static final Logger logger = LogManager.getLogger(TaskBotTypeDemoCommand.class);
  ...
  public Optional<Value> execute(GlobalSessionContext globalSessionContext,
    ...
    TaskBotTypeDemo command = new TaskBotTypeDemo();
    ...
    try {
      Optional<Value> result =  Optional.ofNullable(command.runbot((Taskbot)convertedParameters.get("taskbot")));
      return logger.traceExit(result);
    }
    catch (ClassCastException e) {
      throw new BotCommandException(MESSAGES_GENERIC.getString("generic.IllegalParameters","runbot"));
    }
